...
event_data = {
    "Event ID": list(range(1, 100001)),

    "Event Type": [maybe_nan(random.choice(list(event_types_and_prices.keys()))) for _ in range(100000)],

    # Most events within 0-20km, some up to 100 km
    "Distance (km)": [maybe_nan(round(random.uniform(0, 30), 2)) if random.random() > 0.1
                      else maybe_nan(round(random.uniform(20, 1000), 2)) for _ in range(100000)],
    ...
    "Popularity": [maybe_nan(round(random.gauss(1000, 800))) if random.random() > 0.2
                   else maybe_nan(round(random.uniform(500, 5000))) for _ in range(100000)],

    "Price ($)": [],

    # Majority shorter events, few long ones
    "Duration (hrs)": [maybe_nan(round(random.uniform(0.5, 6), 2)) if random.random() > 0.2
                       else maybe_nan(round(random.uniform(6, 72), 2)) for _ in range(100000)]
}
...
csv_file_path = os.path.join(current_directory, "(nan)100000_generated_events.csv")  # Set full path with filename
